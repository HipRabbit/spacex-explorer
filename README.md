# 🚀 SpaceX Explorer

Ein interaktives Dashboard zur Visualisierung von SpaceX-Missionsdaten, entwickelt als Semesterprojekt für das Modul Web-Programmierung.

## 🔗 Live Demo
Hier klicken: https://hiprabbit.github.io/spacex-explorer/
## 📖 Features
* **Live Countdown:** Nächster Start in Echtzeit.
* **Missions-Archiv:** Übersicht vergangener Starts mit Filter (Jahr/Rakete).
* **Orbit Map:** CSS-basierte Visualisierung der Umlaufbahnen.
* **Raketen-Flotte:** Technische Daten zu Falcon 9, Falcon Heavy & Starship.
* **Statistik:** Hybride Auswertung (Historische Daten + Live API).

## 🛠 Technologie
* **Frontend:** HTML5, CSS3, JavaScript (ES6+)
* **Framework:** Bootstrap 5.3
* **APIs:** The Space Devs API & SpaceX API v4
* **Charts:** Chart.js

## 📝 Lizenz
Erstellt für Bildungszwecke.
